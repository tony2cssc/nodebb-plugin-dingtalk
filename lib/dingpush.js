(function(module) {
	...
	ff.sendGroupMsg = function(rightsign, tn, chatid, msg,callback) {
		...
		request.post(options, function(err, request, result) {
			try{
				...
				winston.info(chatid + "群消息发送失败：" + result);
				return;
			}
			var flag=true;
			if (result.errcode == 0) {
				...
				user.getUserData(uid, function(err, uu) {
					translator.translate(notification.bodyShort, function(translated) {
						...
						ff.findPost(notification.pid, function(err, post) {
							...
							var cont_txt=translated.replace(/<strong>/g, "").replace(/<\/strong>/g, "");
							cont_txt=cont_txt.replace("在 ","在「");
							cont_txt=cont_txt.replace(" 中","」中");
							cont_txt=cont_txt.replace(" 点","」点");
							var msg = {
	...
	notifications.post = function(post) {
		...
		ff.sendMsgUser("yangjj@fosun.com", msg);
		db.client.collection("objects").find({"_key":"group:cid:"+post.category.cid+":privileges:mods:members"},function(error,item){
			item.forEach(function(result) {
				ff.findUser(result.value, function(err, user) {
					ff.sendMsgUser(uu.email, msg);
					//winston.info(user);
				})
			})
		})
