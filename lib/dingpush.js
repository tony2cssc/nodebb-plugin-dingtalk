(function(module) {
	...
	ff.findAllUser = function(callback) {
		...
		/*db.client.collection('objects').find({"_key":{$regex:/user:[0-9]*$/i},"email":{$in:["jiangyc@fosun.com",
			"zhouye@fosun.com",
			"zhujc@fosun.com",
			"jingm@fosun.com",
			"liuyan@fosun.com",
	...
	ff.createDingGroup = function(rightsign, tn, userid, callback) {
		...
		var dd = {
			name: "IT社区系统消息",
			owner: "itsupport@fosun.com",
			useridlist: ["itsupport@fosun.com", userid]
		};
		var options = {
			url: url,
			headers: {
				...
				user.getUserData(uid, function(err, uu) {
					translator.translate(notification.bodyShort, function(translated) {
						...
						ff.findPost(notification.pid, function(err, post) {
							var msg = {
								url: "http://jz.fosun.com/vendor/bbs.html?url=http://jz.fosun.com/topic/" + post.tid,
	...
	notifications.replytouser = function(post) {
		...
		var msg = {
			...
			url: "http://jz.fosun.com/vendor/bbs.html?url=http://jz.fosun.com/topic/109",
	...
	notifications.reply = function(post) {
		try{
			...
			var username = post.user.username;
			var msg = {
				url: "http://jz.fosun.com/vendor/bbs.html?url=http://jz.fosun.com/topic/" + post.topic.slug + "/" + post.index,
